...
for (i in 1:52) 
{
  ...
  moving_average <- SMA(week$Global_intensity, 9)
...
smoothenedWeeksDf$average_smoothened_week <- rowMeans(smoothenedWeeksDf, na.rm=T)
smoothenedWeeksDf <- tail(smoothenedWeeksDf, -8) # remove the first 8 NA values produced from sma
...
which.max(euclideanDistances)   # week 52

# Least anomalous week (Min euclidean distance)
which.min(euclideanDistances)   # week 30

# store scores in a dataframe
scoresDf <- data.frame(week = 1:52, score = euclideanDistances)

# add datetime column for the x axis
smoothenedWeeksDf$Datetime <- with(DataDf, as.POSIXct(paste(Date, Time), format="%d/%m/%Y %H:%M:%S"))[9:10080]
